...
class ScannerAgent(Agent):
    ...
    def fetch_deals(self, memory) -> List[ScrapedDeal]:
        ...
        # Handle both URL strings and Opportunity objects in memory
        if memory:
            # Check if memory contains strings (URLs) or objects with .deal.url attribute
            if isinstance(memory[0], str):
                urls = memory  # memory is already a list of URL strings
            else:
                urls = [opp.deal.url for opp in memory]  # memory contains Opportunity objects
        else:
            urls = []
    ...
    def scan(self, memory: List=[]) -> Optional[DealSelection]:
        ...
        :param memory: a list of URLs (strings) or Opportunity objects representing deals already processed
